...
which is more or less ubuntu 24.04. Docker didn't have an official installation guide for the latest version. 
...
Docker was installed, but I still couldn't run commands without <code>sudo</code> which I realized was happening 
...
I decided on [a simple golang todo webserver](github.com/craniacshencil/got_to_do) as my first project to dockerize.
It involved connecting to postgres but my tcp connections were getting refused. 
```sh
dial tcp [::1]:5432: connect: connection refused
```
It turns out my docker container could not connect to postgres.
...
They are more or less like "imaginary machines" that have their own IP addresses. Postgres (for security purposes) 
...
Find the IPs of the docker bridges
```sh
docker network ls
```
...
This opens up postgres to listen to all the network interface (ones listed using ifconfig) instead of just localhost.
...
## Write about allowing tcp connection using ufw
...
Now the docker containers should start running for you. If they don't you need to reboot, postgresql and ufw are system services which probably need a restart (not the services, your system) to apply new rules.
